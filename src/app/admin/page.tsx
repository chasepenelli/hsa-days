          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-left">
                ...
                <th className="px-4 py-3 font-semibold text-text-muted hidden md:table-cell">Dog</th>
                <th className="px-4 py-3 font-semibold text-text-muted hidden lg:table-cell">Source</th>
                <th className="px-4 py-3 font-semibold text-text-muted hidden md:table-cell">Joined</th>
              </tr>
            </thead>
            <tbody>
              {(recentSubscribers || []).map((sub) => (
                <tr key={sub.id} className="border-b border-border/50 last:border-0">
                  <td className="px-4 py-3 text-text">{sub.name || "—"}</td>
                  <td className="px-4 py-3 text-text-muted">{sub.email}</td>
                  <td className="px-4 py-3 text-text-muted hidden md:table-cell">{sub.dog_name || "—"}</td>
                  <td className="px-4 py-3 text-text-muted hidden lg:table-cell">{sub.signup_source}</td>
...
function StatCard({ label, value, detail }: { label: string; value: number; detail: string }) {
  return (
    <div className="bg-white border border-border rounded-xl p-5">
      <div className="text-[0.7rem] uppercase tracking-[0.08em] text-text-muted font-semibold mb-1">{label}</div>
